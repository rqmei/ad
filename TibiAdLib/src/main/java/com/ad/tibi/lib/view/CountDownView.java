...
public class CountDownView extends View {
     ...
     * 字体大小
     */
    private final int defTextSize = 12;
    /**
     * 倒计时进度条颜色
     */
     ...
     * dp
     */
    private final int defProgressWidth = 3;

    /**
     * 默认倒计时总时长
     * 单位 ms
     */
    private final int defTime = 5000;
    /**
     * 默认倒计时圆角
     * 单位 dp
     */
    private final int defRadius = 25;
    ...
    public CountDownView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        progressWidth = typedArray.getDimensionPixelSize(R.styleable.CountDownView_time_pro_width,
        ...
        textSize = typedArray.getDimensionPixelSize(R.styleable.CountDownView_time_text_size,
                DensityUtils.sp2px(context, defTextSize));

        totalTime = typedArray.getInteger(R.styleable.CountDownView_time_num, defTime);

        radius = typedArray.getDimensionPixelSize(R.styleable.CountDownView_time_radius,
    ...
    protected void onDraw(Canvas canvas) {
        ...
        //画进度条
        mRectF.set(getWidth() / 2 - radius + progressWidth / 2,
                getHeight() / 2 - radius + progressWidth / 2,
                getWidth() / 2 + radius - progressWidth / 2,
                getHeight() / 2 + radius - progressWidth / 2);
        canvas.drawArc(mRectF, 270, sweepAngle, false, proPaint);
        //画文本
        textPaint.getTextBounds(content, 0, content.length(), mBounds);
        canvas.drawText(content, getWidth() / 2 - mBounds.width() / 2,
                getHeight() / 2 + mBounds.height() * 2 / 5, textPaint);
    }
